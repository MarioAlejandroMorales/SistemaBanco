...
public class ventanaApertura extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ventanaApertura frame = new ventanaApertura();
	...
	public ventanaApertura() {
		setTitle("Systemoney(Apertura de cuentas)");
		setIconImage(Toolkit.getDefaultToolkit().getImage(ventanaApertura.class.getResource("/imagenes/LogoIcon.png")));
		int longCurp=18;
		int longNCuenta=9;
		int longTCuenta=40;
		int longCVV=5;
		...
		btnAceptar.addActionListener(new ActionListener() {//////////////////////boton aceptar
			...
			public void actionPerformed(ActionEvent arg0) {
				boolean exist=false;
				exist=conexion.clienteValido(textCurp.getText());
				if(exist==true)
				{
					try
					{
				conexion.insertarPlan(Integer.parseInt(textNCuenta.getText()), textCurp.getText(), 
						textTCuenta.getText(), Integer.parseInt(textIntereses.getText()), 
						Float.parseFloat(textSaldo.getText()), Integer.parseInt(textCVV.getText()));
					}
					catch(Exception PKRepetida)
					{
						JOptionPane.showMessageDialog(null, "Por favor verifique que el numero de cuenta no este en uso");
					}
				}
				else
				{
					JOptionPane.showMessageDialog(null, "Por favor ingrese un cliente valido");
				}
